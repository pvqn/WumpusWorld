...
def draw(visited_position, player_position, map_state):
    ...
    for cell in visited_position:
        ...
        if map_state[cell[0]][cell[1]] & (1 << G) and map_state[cell[0]][cell[1]]!=0: 
            pygame.draw.rect(win, GOLD, (cell[1]*CELL_SIZE, cell[0]*CELL_SIZE, CELL_SIZE, CELL_SIZE))
        

        if map_state[cell[0]][cell[1]] & (1 << B) and map_state[cell[0]][cell[1]] & (1 << S) and map_state[cell[1]][cell[0]]!=0:
            text = font.render('bs', True, (0, 0, 255))
            win.blit(text, (cell[1]*CELL_SIZE, cell[0]*CELL_SIZE))
        if map_state[cell[0]][cell[1]] & (1 << B) and map_state[cell[0]][cell[1]]!=0:
            text = font.render('b', True, (0, 0, 255))
            win.blit(text, (cell[1]*CELL_SIZE, cell[0]*CELL_SIZE))
        if map_state[cell[0]][cell[1]] & (1 << S) and map_state[cell[0]][cell[1]]!=0:
...
def CheckState( current_position, key ):
    ...
    if( ( map_state[current_x][current_y] & ( 1 << key ) ) != 0 ): 
        if key == G or key ==W: 
...
def MoveOneStep( position ):
    ...
    gold=CollectGold( agent_position )
    if gold: 
        draw(visited_position, agent_position, map_state)
        time.sleep(0.5)
...
def PreProcess() :
    ...
    # Start percept
    player_position = agent_position
    visited_position.add(agent_position)
    player_direction = '>'
